	...
	<ul>
		<li class="att popup-wrap"><a id="hd_singin_but_open" href="" class="btn-toggle"><?=GetMessage("AUTH_LOGIN_LINK_TEXT")?></a>
			<form class="frm-login popup-block" name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
				...
				<div class="frm-row">
                    <noindex><a href="<?=$arResult["AUTH_FORGOT_PASSWORD_URL"]?>" class="btn-forgot" rel="nofollow"><?=GetMessage("AUTH_FORGOT_PASSWORD_2")?></noindex></a>
				</div>
				<div class="frm-row">
					<div class="frm-chk">
...
<nav class="menu-block">
    <ul>
        ...
        <li><a href="<?=$APPLICATION->GetCurPage().'?logout=yes'?>">Выйти</a>
        </li>
    </ul>
</nav>
<?endif?>
